...
void setup() {
  ...
  //assembly_one.bme280_assembly();
  //assembly_one.PID_controller_assembly();
  xTaskCreate(task_bme280,"bme280",4096,&assembly_one,1,NULL);
  xTaskCreate(task_PID_controller,"PID controller",4096,&assembly_one,1,NULL);
  xTaskCreate(task_Fan_3pin,"Fan 3 pin",4096,&assembly_one,1,NULL);
